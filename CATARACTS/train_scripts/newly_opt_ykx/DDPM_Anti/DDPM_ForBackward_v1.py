...
class DiffusionUnetHybridImagePolicy(nn.Module):
    def __init__(self, 
        ...
        if opts.Obs == 'LSTM':
            ...
            if loadpretrain:
                # p = '/memory/yangkaixiang/SurgicalSceneUnderstanding/pitfalls_bn/output/checkpoints/anticipation/20240905-2001_BNhori5_cuhk6020Split_lstm_convnext_bs1_seq64_e2e_shuffle_CHE/models/checkpoint_current.pth.tar'
                p = '/memory/yangkaixiang/SurgicalSceneUnderstanding/pitfalls_bn/output/checkpoints/anticipation/20241027-1928_BN_PhaseToolAnti_horizon5_cuhk6020Split_lstm_convnext_bs1_seq64_e2e_shuffle_CHE/models/checkpoint_current.pth.tar'
                weights = torch.load(p, 'cpu')['state_dict']
                new_weight = {}
                for k in obs_encoder.state_dict().keys():
                    if k.startswith('linearout'):
                        new_weight[k] = obs_encoder.state_dict()[k]
                    if k.startswith('lstm.lstm'):
                        new_weight[k] = weights['temporal_head' + k[4:]]
                    elif k.startswith('resnet18'):
                        new_weight[k] = weights[k.replace('resnet18', 'cnn')]
                obs_encoder.load_state_dict(new_weight)
    ...
    def predict_action(self, nobs, To_use_for_lstm=None) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            if self.opts.Obs == 'LSTM':
                cond_data = torch.zeros(size=(B, To, Da), device=device, dtype=dtype)
            else:
    ...
    def compute_loss(self, nobs, nactions, phase_nactions=None):
        ...
        if self.obs_as_global_cond:
            ...
            nobs_features, lstm_features = self.obs_encoder(this_nobs) # [B*To,66]
        ...
        ### 2024.10.25 Uniform Noise ###
        # range_width = 1 * (12 ** 0.5)
        # a = -range_width / 2
        # b = range_width / 2
        # noise = torch.empty(trajectory.shape, device=trajectory.device).uniform_(a, b)
        ...
        if self.CNN_output_loss:
            ...
            target_reg = trajectory
            target_cls = torch.where((trajectory < 1) & (trajectory > 0),torch.cuda.FloatTensor([2]),trajectory).type(torch.cuda.LongTensor)
