...
from .DDPM_ObsEncoder import ObsEncoder
from .DDPM_ObsEncoder_DACAT import ObsEncoder_LSTM
...
class DiffusionUnetHybridImagePolicy(nn.Module):
    def __init__(self, 
        ...
        if self.CNN_output_loss:
            ...
            if opts.num_classes == 7:
                self.criterion_phase = nn.CrossEntropyLoss(reduction='mean',weight=weight)
            else:
                self.criterion_phase = nn.CrossEntropyLoss(reduction='mean')
        ...
        # #### OphNet
        # if opts.split == 'OphNet':
        #     long_net_pretrain_path = f'/memory/yangkaixiang/SurgicalSceneUnderstanding/pitfalls_bn/output/checkpoints/phase/20250511-1618_ICCVReOphNet_OphNetSplit_lstm_convnext_bs1_seq256_frozen_CHT/models/checkpoint_best_acc.pth.tar'
        #     pretrain_checkp = torch.load(long_net_pretrain_path, 'cpu')['ema_state_dict']
        #     unet_ckp = {}
            
        #     for k, v in pretrain_checkp.items():
        #         if k.startswith('model.'):
        #             unet_ckp[k[len('model.'):]] = v
                
        #     self.model.load_state_dict(unet_ckp)
    ...
    def predict_action(self, nobs, To_use_for_lstm=None) -> Dict[str, torch.Tensor]:
        ...
        if self.CNN_output_loss:
            ...
            outputp = (self.obs_encoder.out_layer(lstm_features[0]) + self.obs_encoder.CA_out_layer(lstm_features[1])) / 2
            # outputp = self.obs_encoder.out_layer(lstm_features[0])
    ...
    def gt_cache_reset(self, target): # only for CHT
        # self.obs_encoder.long_cache_reset() # DACAT Long Cache Reset
