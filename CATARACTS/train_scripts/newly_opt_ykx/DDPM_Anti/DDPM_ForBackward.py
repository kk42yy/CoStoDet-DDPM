...
class DiffusionUnetHybridImagePolicy(nn.Module):
    def __init__(self, 
        ...
        if self.CNN_output_loss:
            self.ConvNeXt_LSTM_outlinear = nn.Linear(512, self.opts.num_classes)
            weight = torch.Tensor([
                1.6411019141231247,
                0.19090963801041133,
                1.0,
            ...
            if opts.num_classes == 7:
                ...
                self.criterion_phase = nn.CrossEntropyLoss(reduction='mean') # 2025.5.8
        else:
    ...
    def predict_action(self, nobs, To_use_for_lstm=None) -> Dict[str, torch.Tensor]:
        ...
        if self.CNN_output_loss:
            ...
            action_pred = outputp
            # outputp_mx = torch.max(outputp, dim=2)[0][...,None]
            # action_pred = action_pred * 0.6 + outputp / outputp_mx * 0.4 # 20250514 ICCV Rebuttal
        else:
            # run sampling
            nsample = self.conditional_sample(
                cond_data, 
