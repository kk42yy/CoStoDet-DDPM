...
class ResNet50GN(nn.Module):
    def __init__(self, backbone='convnext', pretrain=True, freeze=False):
        super().__init__()
        self.featureNet = group_norm.resnet50_gn(pretrained=pretrain)
...
class ObsEncoder_LSTM(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        if 'convnext' in opts.backbone.lower():
            self.resnet18 = ConvNeXt(backbone=opts.backbone, pretrain=pretrain, freeze=opts.freeze)
        else:
            self.resnet18 = ResNet50GN(pretrain=pretrain, freeze=opts.freeze)
        self.To = To
