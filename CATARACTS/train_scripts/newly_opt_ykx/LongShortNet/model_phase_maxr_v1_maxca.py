...
class PhaseModel(nn.Module):
	...
	def __init__(self,opts,train=True):
		...
		long_net_pretrain_path = '.../BNPitfalls_CATARACTS_Recognition.pth.tar'
		...
		if self.train:
			...
			# long_net_pretrain_path = os.path.join(os.path.dirname(__file__), f'long_net_{opts.backbone}_pretrain.pth.tar')
