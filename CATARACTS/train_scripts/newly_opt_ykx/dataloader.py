...
def prepare_dataset(opts):
	...
	elif opts.split=='cuhk2525':
		op_paths.sort(key=os.path.basename)
		#print('train')
		train_set = load_data(op_paths[00:25],opts,data_aug=data_aug,shuffle=opts.shuffle)
		#print('val')
		val_set   = load_data(op_paths[25:50],opts,data_aug=False,shuffle=False)
		#print('test')
		test_set  = load_data(op_paths[25:26],opts,data_aug=False,shuffle=False)
...
class Cholec80(Dataset):
	def __init__(self, image_path, ID, opts, data_aug=False, seq_len=1):
		...
		if opts.task == 'phase':
			...
			annotation_path = os.path.join(opts.annotation_folder,'phase_annotations',"train"+ID+".txt")
			with open(annotation_path, "r") as f:
				self.target = []
				reader = csv.reader(f, delimiter='\t')
				next(reader, None)
				for count, row in enumerate(reader):
					if count % 6 == 0: # the gt has been given by 1 fps, ori is 30 fps, now we have 5 fps
