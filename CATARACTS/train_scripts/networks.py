...
class TemporalCNN(nn.Module):
	...
	# 	x = x.flatten(end_dim=1) # [T,C,H,W]
		
	# 	if len(self.cache) < 16:
	# 		x = self.cnn.featureNet(x)
	# 		self.cache = [x.detach()] * 16
	# 	else:
	# 		x = self.cnn.featureNet(x)
	# 		self.cache = self.cache[1:] + [x.detach()]
		
	# 	cache_x = torch.vstack(self.cache)
	# 	# print(cache_x.shape, B, S)
	# 	cache_x = cache_x.view(B,-1,768)
	# 	# print(cache_x.shape)
	# 	cache_x = self.temporal_head(cache_x)
	...
	def extract_image_features(self,x):
		...
		# # 20240705 add diff feature info
		# # x: [256, 768]
...
# from newly_opt_ykx.ViMamba.ViM import VisionMamba
# from torch.cuda.amp import autocast
# from newly_opt_ykx.xLSTM.xlstm import ViLLayer as xlstm
...
class LSTMHead(nn.Module):
	...
	def __init__(self,feature_size,out_size,train_len,lstm_size=512):
		...
		# 	MambaLayer(64),
		# 	# nn.Linear(feature_size, lstm_size),
		# 	# MambaLayer(lstm_size)
		# )
		# self.vim = VisionMamba(embed_dim=feature_size, num_classes=out_size)
		# self.lstm = xlstm(feature_size)
	...
	def forward(self,x):
		...
		return [x]
		# return [self.vim(x)]
		# return [self.out_layer(self.lstm(x))] # xlstm, mamba
