...
def prepare_output_folders(opts):
	...
	trial_name_full = '{}_{}{}_{}Split_{}_{}_bs{}_seq{}_{}{}{}'.format(
...
class InsRegMetricMeter:
	...
	def update(self,loss,output,target):
		...
		self.pred_per_vid[-1].extend(predicted.cpu().numpy())
		self.target_per_vid[-1].extend(target.cpu().numpy())
	...
	def get_scores(self):
		...
		acc_frames = accuracy_score(all_targets,all_predictions)
		ins_acc = [accuracy_score(all_targets[:,idx],all_predictions[:,idx]) for idx in range(all_targets.shape[1])]

		loss = self.loss / len(all_targets)

		return loss, acc_vid, acc_frames, ins_acc
	...
	def start_new_op(self):
		if self.pred_per_vid:
			self.pred_per_vid[-1] = np.array(self.pred_per_vid[-1])
			self.target_per_vid[-1] = np.array(self.target_per_vid[-1])

		self.pred_per_vid.append([])
		self.target_per_vid.append([])
