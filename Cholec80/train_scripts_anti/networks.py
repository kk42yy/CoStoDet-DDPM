	...
	def __init__(self,out_size,backbone,head,opts):
		...
		if head == 'lstm':
			if not opts.CHE:
				...
				self.temporal_head = LSTMHeadCHE(self.cnn.feature_size,out_size,opts.seq_len)
		elif head == 'tcn':
...
class LSTMHeadCHE(nn.Module):

	def __init__(self,feature_size,out_size,train_len,lstm_size=512):

		super(LSTMHeadCHE, self).__init__()
...
class CNN(nn.Module):
	...
	def __init__(self,out_size,backbone,opts):
		...
		if backbone == 'alexnet':
			...
			self.featureNet = convnext.convnext_large(pretrained=True)
			self.featureNet.head = Identity()
