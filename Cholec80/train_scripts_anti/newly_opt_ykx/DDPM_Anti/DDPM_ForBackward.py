...
class DiffusionUnetHybridImagePolicy(nn.Module):
    def __init__(self, 
        ...
        if self.CNN_output_loss:
            self.ConvNeXt_LSTM_outlinear = nn.Linear(512, action_dim * 4)
            self.criterion = nn.SmoothL1Loss(reduction='mean')
            self.criterion_cls = nn.CrossEntropyLoss(reduction='mean')
        else:
            self.ConvNeXt_LSTM_outlinear = nn.Identity()
        
        self.Phase_joint = opts.Phase_joint
        if self.Phase_joint:
            self.ConvNeXt_LSTM_outlinear_phase = nn.Linear(512, 7)
            weight = torch.Tensor([
                1.6411019141231247,
                0.19090963801041133,
                1.0,
    ...
    def conditional_sample_RandomMask(self, 
            condition_data, condition_mask,
            local_cond=None, global_cond=None,
            generator=None,
            # keyword arguments to scheduler.step
    ...
    def predict_action(self, nobs, To_use_for_lstm=None) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            if self.opts.Obs == 'LSTM':
                #### 241104 last 4 frame ####
                cond_data = torch.zeros(size=(B, To, Da, self.opts.pre_len), device=device, dtype=dtype)
            else:
        ...
        # Combine with CNN
        if self.CNN_output_loss and not self.Phase_joint:
            output = self.ConvNeXt_LSTM_outlinear(lstm_features[0])
            output = output.unsqueeze(0)
            output_reg = output[:,:,-self.action_dim:]
            # action_pred = (action_pred + output_reg) / 2
            action_pred = output_reg
    ...
    def compute_loss(self, nobs, nactions, phase_nactions=None):
        ...
        new_trajectory = torch.zeros((trajectory.shape[0], self.opts.seq_len, trajectory.shape[2], self.opts.pre_len), device=trajectory.device)
        """
        new_trajectory: B, Seq_len=T, 12, pre_len
        trajectory: B, 2 * Seq_len=T, 12
        
        new_trajectory的第 pre_len - 1 ~ [q:q + seq_len]=[q, q+seq_len), q + seq_len = T => q = T - seq_len
        pre_len - 1 ~ q = T - seq_len
        k = (pre_len - 1) + (1 - pre_len + k) ~ T - seq_len + (1 - pre_len + k)
        n_Start = T - seq_len - pre_len + 1
        
        new_trajectory[:, :, :, k] = trajectory[:, n_Start+k:n_Start+k+self.opts.seq_len, :]
        
        """
        n_Start = trajectory.shape[1] - self.opts.seq_len - self.opts.pre_len + 1
        for i in range(self.opts.pre_len):
            new_trajectory[:,:,:,i] = trajectory[:, n_Start+i:n_Start+i+self.opts.seq_len, :]
        trajectory = new_trajectory.clone()
        del new_trajectory
        
        ## 调整obs
        global_cond = nobs_features.reshape(batch_size, -1, 64)[:, -self.opts.seq_len:, :]
        ...
        if self.CNN_output_loss:
            if self.Phase_joint:
                outputp = self.ConvNeXt_LSTM_outlinear_phase(lstm_features[0])
                outputp = outputp.reshape(batch_size, horizon, -1)
                outputp = outputp.transpose(1, 2) # b, 7, horizon
                loss += self.criterion_phase(outputp, phase_nactions)

            # else:
            output = self.ConvNeXt_LSTM_outlinear(lstm_features[0])
            output = output.reshape(batch_size, horizon, -1)
            output_reg = output[:,:,-self.action_dim:]
            output_cls = output[:,:,:-self.action_dim].view(batch_size,-1,3,self.action_dim)
            
            # # 241017 only cls
            # output_cls = output.view(batch_size,-1,3,self.action_dim)
            output_cls = output_cls.transpose(1,2)
            
            target_reg = nactions
            target_cls = torch.where((nactions < 1) & (nactions > 0),torch.cuda.FloatTensor([2]),nactions).type(torch.cuda.LongTensor)
            
            loss += (
                        self.criterion(output_reg, target_reg) + \
                        0.01 * self.criterion_cls(output_cls, target_cls)
                    ) * self.action_dim
        
        return loss
    
    def compute_loss_MaskGTRandom(self, nobs, nactions, phase_nactions=None): # random mask task
        # normalize input
        # nobs = self.normalizer.normalize(nobs)
        # nactions = self.normalizer['action'].normalize(nactions)
        batch_size = nactions.shape[0]
        ...
        masked_trajectory = self.random_mask(trajectory)
        
        # Predict the noise residual
        pred = self.model(masked_trajectory, timesteps, 
            local_cond=local_cond, global_cond=global_cond)

        target = trajectory # recover the randomly masked trajectory (i.e., label)
    ...
    def random_mask(self, x: torch.Tensor, max_mask: float = 1.0):
        shape = x.shape
        num_elements = x.numel()
        mask_percentage = random.uniform(0, max_mask)
        num_masked_elements = int(num_elements * mask_percentage)
        mask = torch.ones(num_elements, dtype=torch.bool, device=x.device)
        mask[torch.randint(0, num_elements, (num_masked_elements,))] = False
