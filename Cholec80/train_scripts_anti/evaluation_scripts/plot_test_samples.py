...
for f in sorted(os.listdir(sample_path)):
	...
	if 'video' in f:
		...
		print('saving', savepath + f.split('.')[0] + '_tool.png')
		plt.savefig(savepath + f.split('.')[0] + '_tool.png', bbox_inches='tight', dpi=300, pad_inches=0.0)
		plt.close()
