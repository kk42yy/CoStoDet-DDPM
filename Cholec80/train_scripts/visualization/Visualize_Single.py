...
def visual_single(pred_pt_p, bmp_pt_p, ptname):
    with open(pred_pt_p, 'r+') as f:
        pred_arr = [int(i.split('\t')[1].split('\n')[0]) for i in f.readlines()[1:]]
   
    pred_colors = np.array([[color_map[i]['color'] for i in pred_arr] for _ in range(100)], dtype=np.uint8)

    # plt.imshow(pred_colors)
    # plt.axis('off')
    ...
    # plt.savefig(bmp_pt_p, bbox_inches='tight')
    # plt.close()
    pred_colors = cv2.cvtColor(pred_colors, cv2.COLOR_RGB2BGR)
    cv2.imwrite(bmp_pt_p, pred_colors)
    print(ptname)
...
if __name__ == "__main__":
    predp = '/memory/yangkaixiang/SurgicalSceneUnderstanding/pitfalls_bn/output/predictions/phase/20240701-1706_trial_cuhknotestSplit_lstm_convnextv2_lr0.0001_bs1_seq256_frozen/pred_off256'
    savep = '/memory/yangkaixiang/SurgicalSceneUnderstanding/pitfalls_bn/output/predictions/phase/20240701-1706_trial_cuhknotestSplit_lstm_convnextv2_lr0.0001_bs1_seq256_frozen/visualpred_off256'
    os.makedirs(savep, exist_ok=True)
    for ID in range(41,81):
        filename = f'video{ID}-phase'
        visual_single(
            f"{predp}/{filename}.txt",
            f"{savep}/{filename}.png",
            filename
        )