	...
	def forward(self,x,long_term=None):
		...
		if long_term is not None:
			...
			if not self.trainee:
				...
				## v1 240720 seem not useful, may the diff between train and test
				...
				idx = multi_response.max(dim=0)[-1].long()
			# 	# single_response = (x.mean(dim=1, keepdim=True) * long_term).mean(dim=-1) # [1, t], fs_t with f_i
			# 	# multi_response =  torch.flip(torch.cumsum(torch.flip(single_response[0],dims=[0]),dim=0), dims=[0]) # the suffix accumulate
			# 	# idx = multi_response.max(dim=0)[-1].long()
			# idx = -100
...
class PhaseModel(nn.Module):
	...
	def __init__(self,opts,train=True):
		...
		long_net_pretrain_path = os.path.join(os.path.dirname(__file__), f'long_net_{opts.backbone}.pth.tar')
		...
		if self.Train:
			...
			# # 20240825
			# # short term lstm and linear use the same weight as long term and long_net
			# for k in short_parameter.keys():
			# 	if "temporal" in k:
			# 		short_parameter[k.replace("temporal", "CA_temp")] = deepcopy(checkp_for_short[k])
			# 		print(k, k.replace("temporal", "CA_temp"))
					
			short_parameter.update(checkp_for_short)
			self.net_short.load_state_dict(short_parameter)
			self.net_long.requires_grad_(False)
			self.net_short.temporal_head.requires_grad_(False)
