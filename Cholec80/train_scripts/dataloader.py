...
def prepare_dataset(opts):
	...
	if opts.split=='old':
		...
		train_set = load_data(op_paths[00:40],opts,data_aug=data_aug,shuffle=opts.shuffle)
		#print('val')
		val_set   = load_data(op_paths[40:80],opts,data_aug=False,shuffle=False)
		#print('test')
		test_set  = load_data(op_paths[42:43],opts,data_aug=False,shuffle=False)
		...
		#print('val')
		val_set   = load_data([op_paths[int(i)-1] for i in testid],opts,data_aug=False,shuffle=False)
		#print('test')
		test_set  = load_data(op_paths[41:42],opts,data_aug=False,shuffle=False)
	...
	if shuffle:
		...
		for op_path in op_paths:
			...
			if os.path.isdir(op_path):
				dataset = Cholec80(op_path, ID, opts, data_aug, opts.seq_len*2)
				data.append(dataset)
